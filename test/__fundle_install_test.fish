...
function setup
	...
	__fundle_gitify $path/fixtures/foo/with_dependency
	__fundle_gitify $path/fixtures/foo/with_init
	__fundle_plugin 'foo/with_dependency' $path/fixtures/foo/with_dependency
...
test "$TESTNAME: succeeds when all plugins exist"
	(__fundle_cleanup_plugins;
		and __fundle_plugin 'foo/with_init' $path/fixtures/foo/with_init;
