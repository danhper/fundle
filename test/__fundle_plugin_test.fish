...
@test "$TESTNAME: uses default url when not given" (__fundle_get_url 'foo/bar'
	 __fundle_get_url 'foo/path-flag') = $__fundle_plugin_urls[1] $__fundle_plugin_urls[4]

@test "$TESTNAME: uses given url" '/url/for/baz' \
	'/url/for/url-flag' \
...
@test "$TESTNAME: uses given path flag" 'foo/path-flag:path4' \
